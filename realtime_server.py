...
# Add middleware to disable caching in development
@app.middleware("http")
...
async def websocket_endpoint(websocket: WebSocket):
    ...
    async def initialize_openai(model: str = None):
        ...
        try:
            ...
            # Use exception logging to capture empty-string errors and stack traces
            logger.exception("Failed to connect to OpenAI: %r", e)
            openai_ready.clear()  # Ensure flag is cleared on failure
            await websocket.send_text(json.dumps({
                "type": "error",
                "content": f"Failed to initialize OpenAI connection: {type(e).__name__}: {e}"
            }, ensure_ascii=False))
            return False
